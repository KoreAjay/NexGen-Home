...
$name = $specialty = $contact_email = $contact_phone = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate inputs
    if (empty($name)) {
        $message = "Error: Technician name cannot be empty.";
    } elseif (empty($specialty)) {
        $message = "Error: Specialty cannot be empty.";
    } elseif (!empty($contact_email) && !filter_var($contact_email, FILTER_VALIDATE_EMAIL)) {
        $message = "Error: Invalid email format.";
    } else {
        // Prepare an insert statement
        $sql = "INSERT INTO technicians (name, specialty, contact_email, contact_phone) VALUES (?, ?, ?, ?)";

        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ssss", $name, $specialty, $contact_email, $contact_phone);
            ...
            if ($stmt->execute()) {
                ...
                $message = "Error adding technician: " . $stmt->error;
            }
            $stmt->close();
        } else {
...
<head>
    ...
    <title>Add New Technician - Admin Panel</title>
...
<body>
    <header>
        ...
        <nav class="admin-navbar">
            <ul>
                ...
                <li><a href="admin_add_technician.php" class="active"><i class="fas fa-user-plus"></i> Add Technician</a></li>
    ...
    <div class="container">
        ...
        <p class="subtitle">Add new service technicians to your team.</p>
        ...
        <div class="form-card">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <div class="form-group">
                    <label for="specialty">Specialty (e.g., Plumbing, Electrical)</label>
                    <input type="text" name="specialty" id="specialty" value="<?php echo htmlspecialchars($specialty ?? ''); ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" name="contact_phone" id="contact_phone" value="<?php echo htmlspecialchars($contact_phone ?? ''); ?>">
                </div>
                <button type="submit" class="btn btn-primary"><i class="fas fa-user-plus"></i> Add Technician</button>
            </form>
        </div>
    </div>
</body>
